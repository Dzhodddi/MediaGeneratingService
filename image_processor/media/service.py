...
class MediaService:
    ...
    async def _prepare_assets(self, payload: CreateMediaSchema):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            results = await asyncio.gather(*(v_tasks + a_tasks))
            ...
            temp_files.extend(v_paths + a_paths)

            return video_map, a_paths, temp_files

    @staticmethod
    def _get_part_args(video_paths: list, audio_path: str, output_filename: str):
        v_streams = [
        ...
        a = ffmpeg.input(audio_path, stream_loop=-1).audio.filter("volume", 0.2)
    ...
    async def _generate_part(
        self, index: int, video_paths: list, audio_path: str
    ) -> str:
        part_filename = f"/tmp/part_{index}.mp4"

        process = await asyncio.create_subprocess_exec(
            *self._get_part_args(video_paths, audio_path, part_filename),
    ...
    async def process_task(self, payload: CreateMediaSchema):
        video_map, audio_list, all_assets = await self._prepare_assets(payload)
        generated_parts = []
        ...
        try:
            ...
            for i, video_combo in enumerate(combinations):
                ...
                part_path = await self._generate_part(
                    i, list(video_combo), selected_audio
                )
